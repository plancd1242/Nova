...
class Settings:
    ...
    led_count: int = _int("NOVA_LED_COUNT", 16)
    ...
    led_freq_hz: int = _int("NOVA_LED_FREQ_HZ", 800000)
    led_dma: int = _int("NOVA_LED_DMA", 10)
    led_invert: bool = _bool("NOVA_LED_INVERT", False)
    led_channel: int = _int("NOVA_LED_CHANNEL", 0)
    ...
    voice_commands_enabled: bool = _bool("NOVA_VOICE_COMMANDS_ENABLED", False)
    voice_wake_word_enabled: bool = _bool("NOVA_VOICE_WAKE_WORD_ENABLED", False)
    voice_wake_words: str = os.getenv("NOVA_VOICE_WAKE_WORDS", "hey nova,nova")
    voice_transcription_engine: str = os.getenv("NOVA_VOICE_TRANSCRIPTION_ENGINE", "vosk")
    vosk_model_path: str = os.getenv("NOVA_VOSK_MODEL_PATH", "models/vosk-model-small-en-us-0.15")
    vosk_model_auto_download: bool = _bool("NOVA_VOSK_MODEL_AUTO_DOWNLOAD", False)
    vosk_model_url: str = os.getenv("NOVA_VOSK_MODEL_URL", "https://alphacephei.com/vosk/models/vosk-model-small-en-us-0.15.zip")
    audio_input_device: str = os.getenv("NOVA_AUDIO_INPUT_DEVICE", "")
    voice_sample_rate: int = _int("NOVA_VOICE_SAMPLE_RATE", 16000)
    voice_record_seconds: float = env_float("NOVA_VOICE_RECORD_SECONDS", 5.0)
    voice_command_timeout_seconds: float = env_float("NOVA_VOICE_COMMAND_TIMEOUT_SECONDS", 8.0)
    voice_silence_seconds: float = env_float("NOVA_VOICE_SILENCE_SECONDS", 1.2)
    voice_wake_cooldown_seconds: float = env_float("NOVA_VOICE_WAKE_COOLDOWN_SECONDS", 2.0)
    ...
    router_control_enabled: bool = _bool("NOVA_ROUTER_CONTROL_ENABLED", False)
    router_automation_engine: str = os.getenv("NOVA_ROUTER_AUTOMATION_ENGINE", "playwright")
    router_url: str = os.getenv("NOVA_ROUTER_URL", "")
    router_model: str = os.getenv("NOVA_ROUTER_MODEL", "")
    router_local_password: str = os.getenv("NOVA_ROUTER_LOCAL_PASSWORD", "")
    router_require_ethernet: bool = _bool("NOVA_ROUTER_REQUIRE_ETHERNET", True)
    router_require_off_confirmation: bool = _bool("NOVA_ROUTER_REQUIRE_OFF_CONFIRMATION", True)
    router_headless: bool = _bool("NOVA_ROUTER_HEADLESS", True)
    router_timeout_seconds: int = _int("NOVA_ROUTER_TIMEOUT_SECONDS", 45)
    router_debug_screenshots: bool = _bool("NOVA_ROUTER_DEBUG_SCREENSHOTS", False)
    router_default_smart_connect: bool = _bool("NOVA_ROUTER_DEFAULT_SMART_CONNECT", False)
    router_default_ofdma: bool = _bool("NOVA_ROUTER_DEFAULT_OFDMA", True)
    router_default_main_24: bool = _bool("NOVA_ROUTER_DEFAULT_MAIN_24", True)
    router_default_main_5g1: bool = _bool("NOVA_ROUTER_DEFAULT_MAIN_5G1", True)
    router_default_main_5g2: bool = _bool("NOVA_ROUTER_DEFAULT_MAIN_5G2", True)
